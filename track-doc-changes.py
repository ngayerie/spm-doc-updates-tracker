...
CATEGORIES = {
    ...
    "cf1": {
        ...
        "products": {
            ...
            # Legacy product directory names retained so historical months
            # (before the 2026 "Magic" -> "Cloudflare" doc renames) still resolve.
...
def run_git_command(cmd):
    ...
    result = subprocess.run(
        cmd, capture_output=True, text=True, shell=True, cwd=REPO_PATH
    )
...
def get_commits_in_range(start_date, end_date, product_paths):
    ...
    for line in output.split("\n"):
        if line:
            ...
            commits.append(
                {"hash": hash_val, "subject": subject, "date": date, "author": author}
            )
...
def is_trivial_change(commit_subject):
    ...
    return any(
        re.search(pattern, subject_lower, re.IGNORECASE) for pattern in TRIVIAL_PATTERNS
    )
...
def get_changelog_entries_in_date_range(start_date, end_date, tracked_products):
    ...
    for product in changelog_by_product:
        changelog_by_product[product].sort(
...
def main():
    ...
    parser.add_argument(
        "--repo", type=str, required=True, help="Path to cloudflare-docs repository"
    )
    parser.add_argument(
        "--month", type=str, help="Month in YYYY-MM format (default: last month)"
    )
    parser.add_argument(
        "--category",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Specific products to track (overrides --category)",
    )
    parser.add_argument(
        "--include-trivial", action="store_true", help="Include trivial changes"
    )
    ...
    if args.products:
        ...
        tracked_products = {
            k: v for k, v in tracked_products.items() if k in products_to_track
        }
    else:
    ...
    changelog_by_product = get_changelog_entries_in_date_range(
        start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), tracked_products
    )

    total_changelog_entries = sum(
        len(entries) for entries in changelog_by_product.values()
    )
